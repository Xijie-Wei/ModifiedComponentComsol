...
bool ComponentComsol::Initialise(const std::string &mesh,
  ...
  if (unit == "") {
    // Length unit not specified by the user. 
    // Try to read it from the header of the "field" file. 
    std::ifstream ff(field);
    if (!ff) {
      throw Exception("Could not open file " + field +
                      " for reading.\nThe file perhaps does not exist");
    }
    for (std::string line; std::getline(ff, line);) {
      if (!startsWith(line, "%")) break;
      if (!startsWith(line, "% Length unit:")) continue;
  ...
  if (!fmplist)
    throw Exception("Could not open file " + mplist +
                    " for reading.\nThe file perhaps does not exist");
  ...
  if (!fmesh)
    throw Exception("Could not open file " + mesh +
                    " for reading.\nThe file perhaps does not exist");
  ...
  } while (!endsWith(line, "# number of mesh points") &&
           !endsWith(line, "# number of mesh vertices"));
  ...
  } while (!endsWith(line, "# number of elements"));
  ...
  while (sline >> token) {
    if (token == "V") {
      stoken = token;
      sline >> token; // Looking for "(V)"
      if (token == "(V)") {
        std::cout << m_className << "::Initialise:\n"
                  << "    Reading data for weighting field " << stoken << ".\n";
        wfields.push_back(stoken);
        m_wpot.emplace(stoken, std::vector<double>(m_nodes.size(), 0.));
      }
    }
  }
...
bool ComponentComsol::InitialiseMagnticField(const std::string &mesh,
                                 const std::string &mplist,
                                 const std::string &field,
                                 std::string unit) {
  //ComponentFieldMap::Reset();
  ...
  m_bx.resize(m_nodes.size());
  m_by.resize(m_nodes.size());
  m_bz.resize(m_nodes.size());
  do {
    if (!std::getline(ffield, line)) {
      std::cerr << m_className << "::Initialise:\n"
                << "    Error parsing " << field << ".\n";
  ...
  //m_b0 = {0.,0.,1.};
  Prepare();
  m_EnableMagneticField = true; // enable HasMagneticField;
  std::cout << std::endl << m_className << "::Initialise: Done.\n";
  return true;
}

// Get MagneticField Info
void ComponentComsol::MagneticField(const double xin, const double yin, const double zin,double &bx, double &by, double &bz, int &status){
  bx = by = bz = 0.;
  status = 0;
  ...
  // Check Range
  if(CheckInRange(x,y,z)) status = -6; 

  // Find Element same as E field (Same as ComponentFieldMap)
  double t1, t2, t3, t4, jac[4][4], det;
  int imap = -1;
  if (m_elementType == ElementType::Serendipity) {
    imap = ComponentFieldMap::FindElement5(x, y, t1, t2, t3, t4, jac, det);
  } else if (m_elementType == ElementType::CurvedTetrahedron) {
    imap = ComponentFieldMap::FindElement13(x, y, z, t1, t2, t3, t4, jac, det);
  }
  ...
  if (m_elementType == ElementType::Serendipity) {
    if (m_debug) {
      PrintElement("Magneticfield", x, y, z, t1, t2, t3, t4, imap, dummy);
    }
    if (fabs(det) < 1e-12) {
      std::array<double, 6> Tempbx, Tempby, Tempbz;
      for (size_t i = 0; i < 6; ++i) {
        Tempbx[i] = m_bx[element.emap[i]];
        Tempby[i] = m_by[element.emap[i]];
        Tempbz[i] = m_bz[element.emap[i]];
      }
      bx = ComponentFieldMap::Potential3(Tempbx, {t1, t2, t3});
      by = ComponentFieldMap::Potential3(Tempby, {t1, t2, t3});
      bz = ComponentFieldMap::Potential3(Tempbz, {t1, t2, t3});

    } else {
      std::array<double, 8> Tempbx, Tempby, Tempbz;
      for (size_t i = 0; i < 8; ++i) {
        Tempbx[i] = m_bx[element.emap[i]];
        Tempby[i] = m_by[element.emap[i]];
        Tempbz[i] = m_bz[element.emap[i]];
      }
      bx = ComponentFieldMap::Potential5(Tempbx, {t1, t2});
      by = ComponentFieldMap::Potential5(Tempby, {t1, t2});
      bz = ComponentFieldMap::Potential5(Tempbz, {t1, t2});
    ...
    std::array<double, 10>  Tempbx, Tempby, Tempbz;
    for (size_t i = 0; i < 10; ++i) {
        Tempbx[i] = m_bx[element.emap[i]];
        Tempby[i] = m_by[element.emap[i]];
        Tempbz[i] = m_bz[element.emap[i]];
    }
    bx = ComponentFieldMap::Potential13(Tempbx, {t1, t2, t3, t4});
    by = ComponentFieldMap::Potential13(Tempby, {t1, t2, t3, t4});
    bz = ComponentFieldMap::Potential13(Tempbz, {t1, t2, t3, t4});
  }

  // Transform field to global coordinates.
  UnmapFields( bx, by, bz, x, y, z, xmirr, ymirr, zmirr, rcoordinate, rotation);
  status = 0;
  if (m_debug) {
  std::cout<< "Function MagneticField called, try to get B field at ("<<x<<", "<<y<<", "<<z<<") return:{" << bx <<", "<< by << ", " << bz << " }" <<std::endl;
  }
}
